...
function componentApiJs(isRoot) {
  const lines = [];
  if (isRoot) {
...
function componentApiStubDTS(isRoot) {
  const lines = [];
  lines.push((0, import_common.header)("Generated `api` utility."));
...
async function buildMountTree(ctx, startPush, definitionPath, attributes) {
  ...
  for (const attribute of attributes) {
    ...
    if (!componentExport) {
      (0, import_context.logError)(ctx, `No export found for ${attribute}`);
      return await ctx.crash(1, "fatal");
    }
    const [_, node] = componentExport;
    if (node.type !== "branch") {
      (0, import_context.logError)(ctx, `Expected branch at ${attribute}`);
      return await ctx.crash(1, "fatal");
    }
    current = node.branch;
  }
  ...
  for (const [identifier, componentExport] of exports) {
    if (componentExport.type === "leaf") {
      if (componentExport.leaf.startsWith("_reference/childComponent/")) {
        ...
        if (!childComponent) {
          (0, import_context.logError)(ctx, `No child component found for ${componentName}`);
          return await ctx.crash(1, "fatal");
        }
